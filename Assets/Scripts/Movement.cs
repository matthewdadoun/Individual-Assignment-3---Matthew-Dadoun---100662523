    ...
    private void StateMachine(PlayerState state)
    {
        ...
        switch (state)
        {
            ...
            case PlayerState.RUNNING:
                ...
                //running into jumping
                if (Input.GetButtonDown("Jump") && coll.onGround)
                {
                    currentState = PlayerState.JUMPING;
                }
                //running into falling

                if (rb.velocity.y <= 0.0f&&!coll.onGround)
                {
                    currentState = PlayerState.FALLING;
                }

                //running into dashing
                if (Input.GetButtonDown("Fire1") && !hasDashed)
                {
                    // As long as there is some directional input
                    if (xRaw != 0 || yRaw != 0)
            ...
            case PlayerState.JUMPING:
                ...
                //jumping into falling
                if (rb.velocity.y<=0.0f&&!coll.onWall)
              {
                  currentState = PlayerState.FALLING;
              }
                //jumping into dashing
                if (Input.GetButtonDown("Fire1") && !hasDashed)
                {
                    // As long as there is some directional input
                    if (xRaw != 0 || yRaw != 0)
            ...
            case PlayerState.ON_WALL:
                ...
                //wall sliding into idle
                if (coll.onGround && (xInput <= 0.01f || xInput >= 0.01f))
                {
                    currentState = PlayerState.IDLE;
                }

                //wall sliding into running
                if (xInput > 0.01f || xInput < -0.01f)
                {
                    if (coll.onGround)
                    {
                ...
                //wall sliding into falling 
                if (rb.velocity.y <= 0.0f && !coll.onWall)
                {
                    currentState = PlayerState.FALLING;
                }

                //wall sliding into wall jumping
                if (Input.GetButtonDown("Jump") && coll.onWall)
                {
                    currentState = PlayerState.JUMPING;
                }
            ...
            case PlayerState.DASHING:
                ...
                //Dashing into falling
                if (rb.velocity.y <= 0.0f && !coll.onWall)
                {
                    currentState = PlayerState.FALLING;
                }

                //Dashing into wall sliding
                if ((rb.velocity.x > 0 && coll.onRightWall) || (rb.velocity.x < 0 && coll.onLeftWall))
                {
                    currentState = PlayerState.ON_WALL;
                }
